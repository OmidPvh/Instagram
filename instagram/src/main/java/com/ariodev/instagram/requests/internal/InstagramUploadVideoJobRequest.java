...
{
    private final String TAG = "VideoJobRequest";
    ...
    private RequestBody createRequestBody()
    {
        return new MultipartBody.Builder().setType(MultipartBody.FORM)
                                          .addPart(Headers.of("Content-Disposition", "attachment; filename=\"video.mp4\""), RequestBody.create(MediaType.parse("video/mp4"), videoFile))
                                          .build();
    }
    ...
    public StatusResult execute() throws IOException
    {
        ...
        Response response = this.api.getClient()
                                    .newCall(createHTTPRequest())
                                    .execute();
